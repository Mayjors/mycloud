...
public class SparkTestService {
    ...
    public Map<String, Object> calculateTopTen() {
        ...
        JavaRDD<String> lines = sparkContext.textFile("src/test/java/test,txt").cache();
        ...
        JavaPairRDD<String, Integer> sorted = temp.sortByKey(false).mapToPair(integerStringTuple2 -> new Tuple2<>(integerStringTuple2._2, integerStringTuple2._1));
    ...
    public void sparkExerciseDemo() {
        ...
        JavaRDD<Integer> rdd01 = sparkContext.parallelize(data);
        rdd01 = rdd01.map(num -> {
            return num * num;
        });
        ...
        JavaRDD<Integer> unionRdd = sparkContext.parallelize(data);
        ...
        resul.forEach(x -> {
            log.info("group ny key:{} value:{}", x._1, Joiner.on(",").skipNulls().join(x._2).toString());
        });
    }
    ...
    public void sparkStreaming() throws InterruptedException {
        JavaStreamingContext jsc = new JavaStreamingContext(sparkContext, Durations.seconds(10));   // 批 间隔时间
